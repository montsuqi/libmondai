...
#ifdef DEBUG
static int Depth;
static void DumpIndentLine(void) {
  int i;
...
static void DumpNode(xmlNode *node) {
  xmlChar *text;
...
static void DumpIdTable(GHashTable *ids) {
  void Dump(char *id, xmlNode *node) {
    printf("id = [%s]\n", id);
    DumpNode(node);
  }
...
static void MakeIdTable(GHashTable *ids, xmlNode *a_node) {
  xmlNode *node;
  xmlChar *id;
...
static xmlNodePtr SearchNode(xmlNodePtr tree, char *ns, char *tag, char *prop,
                             char *pvalue) {
  ...
  while (tree != NULL) {
    ...
    case XML_ELEMENT_NODE:
      ...
      } else if ((pvalue == NULL) && (!strcmp(thisns, ns)) &&
                 (!strcmp(XMLName(tree), tag))) {
        ret = tree;
      } else if ((!strcmp(thisns, ns)) && (!strcmp(XMLName(tree), tag)) &&
                 ((thisnn = XMLGetProp(tree, prop)) != NULL) &&
                 (!strcmp(thisnn, pvalue))) {
        ret = tree;
      } else if (XMLNodeChildren(tree) != NULL) {
        ret = SearchNode(XMLNodeChildren(tree), ns, tag, prop, pvalue);
      }
      break;
    case XML_TEXT_NODE:
...
static void _SOAP_UnPackValue(ValueStruct *value, GHashTable *ids,
                              xmlNode *node) {
  int i;
  xmlChar *href, *xsi_null;
  unsigned char *buff;
  xmlChar *text;
  xmlNode *child, *rnode;
  size_t size;
    ...
    if (((xsi_null = XMLGetNsProp(node, "null", XSI_URL)) != NULL) &&
      ...
      case GL_TYPE_BYTE:
        if ((XMLNodeType(node) == XML_ELEMENT_NODE) &&
            ...
            (XMLNodeType(child) == XML_TEXT_NODE) &&
            ((text = XMLNodeContent(child)) != NULL)) {
          DecodeBase64(ValueByte(value), ValueByteLength(value), text,
                       strlen(text));
        }
        break;
      case GL_TYPE_BINARY:
...
static ValueStruct *_SOAP_LoadValue(ValueStruct *upper, GHashTable *ids,
                                    xmlNode *node) {
  xmlChar *href, *tname, *xsi_null, *text;
  xmlNode *rnode, *child;
  ...
  if (node != NULL) {
    ...
    if ((tname = XMLGetNsProp(node, "type", XSI_URL)) != NULL) {
      ...
      if (strcmp(tname, "SOAP-ENC:Array") == 0) {
        ...
        } else if (strcmp(tname, "SOAP-ENC:boolean") == 0) {
          value = NewValue(GL_TYPE_BOOL);
        } else if (strcmp(tname, "SOAP-ENC:base64") == 0) {
    ...
    } else if ((href = XMLGetProp(node, "href")) != NULL) {
      dbgprintf("href  = [%s]\n", href);
      if ((*href == '#') &&
          ((rnode = g_hash_table_lookup(ids, href + 1)) != NULL)) {
...
static size_t _SOAP_PackValue(CONVOPT *opt, unsigned char *p, char *ns,
                              char *name, char *id, ValueStruct *value,
                              char *path) {
  ValueStruct *ival;
  int i;
  unsigned char *pp;
  ...
  if (value != NULL) {
    ...
    case GL_TYPE_ARRAY:
      ...
      for (i = 0; i < ValueRecordSize(value); i++) {
        ...
        p += _SOAP_PackValue(opt, p, ns, name, ibuff, ival, ibuff);
      }
      opt->nIndent--;
      p += IndentLine(opt, p);
      if (ns != NULL) {
    ...
    case GL_TYPE_RECORD:
      ...
      for (i = 0; i < ValueRecordSize(value); i++) {
        ...
        if ((IS_VALUE_STRUCTURE(ival)) || (IS_VALUE_BITS(ival))) {
          p += IndentLine(opt, p);
          p += sprintf(p, "<%s", ValueRecordName(value, i));
          if (ValueType(ival) == GL_TYPE_ALIAS) {
            p += sprintf(p, " href=\"#%s.%s\"/>", path, ValueAliasName(ival));
          } else if (ValueType(ival) == GL_TYPE_ARRAY) {
          ...
          snprintf(ibuff, SIZE_LONGNAME, "%s.%s", path,
                   ValueRecordName(value, i));
          p += _SOAP_PackValue(opt, p, NULL, ValueRecordName(value, i), ibuff,
                               ival, path);
        }
      }
      opt->nIndent--;
      ...
      if (fOut) {
        for (i = 0; i < ValueRecordSize(value); i++) {
          ...
          if ((IS_VALUE_STRUCTURE(ival)) || (IS_VALUE_BITS(ival))) {
            ...
            p += _SOAP_PackValue(opt, p, ns, ValueRecordName(value, i), ibuff,
                                 ival, ibuff);
          }
        }
      }
...
extern size_t SOAP_PackValue(unsigned char *p, ValueStruct *value, char *method,
                             char *prefix, char *uri, Bool fIndent,
                             Bool fBodyOnly) {
