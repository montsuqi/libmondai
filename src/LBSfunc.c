...
extern void LBS_EmitString(LargeByteString *lbs, char *str) {
  ...
  if (left_size < str_size) {
    LBS_Grown(lbs,
              lbs->asize +
                  ((((str_size - left_size) / SIZE_GROWN) + 1) * SIZE_GROWN),
              TRUE);
  }
...
static gboolean eval_cb1(const GMatchInfo *info, GString *res, gpointer data) {
  gchar *match;
...
static gchar *UTF8Normalize(gchar *str) {
  gchar *ret;
