...
static size_t _CSV_UnPackValue(CONVOPT *opt, unsigned char *p,
                               ValueStruct *value, char *buff) {
  int i;
  char *q;
  unsigned char *pp;
...
static size_t CSV_Encode(char *str, char fSsep, char *p) {
  char *pp;
...
static size_t __CSV_PackValue(CONVOPT *opt, unsigned char *p,
                              ValueStruct *value, Bool fOuter, Bool fNsep,
                              Bool fSsep, Bool fCesc, char *buff) {
  int i;
  unsigned char *pp;
  ...
  if (value != NULL) {
    ...
    case GL_TYPE_ARRAY:
      for (i = 0; i < ValueArraySize(value); i++) {
        p += __CSV_PackValue(opt, p, ValueArrayItem(value, i), FALSE, fNsep,
                             fSsep, fCesc, buff);
...
static size_t _CSV_PackValue(CONVOPT *opt, unsigned char *p, ValueStruct *value,
                             Bool fNsep, Bool fSsep, Bool fCesc, char *buff) {
  size_t ret;
  *p = '\0';
...
static size_t _CSV_SizeValue(CONVOPT *opt, ValueStruct *value, Bool fOuter,
                             Bool fNsep, Bool fSsep, Bool fCesc) {
  int i;
  size_t ret;
  char *str;
  char buff[SIZE_BUFF + 1];
  ...
  case GL_TYPE_ARRAY:
    ret = 0;
    for (i = 0; i < ValueArraySize(value); i++) {
      ret += _CSV_SizeValue(opt, ValueArrayItem(value, i), FALSE, fNsep, fSsep,
                            fCesc);
      if ((opt->fNewLine) && (fOuter)) {
        ret += 1;
      }
  ...
  case GL_TYPE_RECORD:
    ret = 0;
    for (i = 0; i < ValueRecordSize(value); i++) {
      ret += _CSV_SizeValue(opt, ValueRecordItem(value, i), FALSE, fNsep, fSsep,
                            fCesc);
    }
    break;
  case GL_TYPE_ALIAS:
...
extern size_t CSV1_SizeValue(CONVOPT *opt, ValueStruct *value) {
  return (_CSV_SizeValue(opt, value, TRUE, TRUE, TRUE, FALSE));
}

extern size_t CSV2_SizeValue(CONVOPT *opt, ValueStruct *value) {
  return (_CSV_SizeValue(opt, value, TRUE, FALSE, FALSE, FALSE));
}

extern size_t CSV3_SizeValue(CONVOPT *opt, ValueStruct *value) {
  return (_CSV_SizeValue(opt, value, TRUE, FALSE, TRUE, FALSE));
}

extern size_t CSVE_SizeValue(CONVOPT *opt, ValueStruct *value) {
...
static void SQL_Encode(char *str, LargeByteString *lbs) {
  for (; *str != 0; str++) {
    switch (*str) {
    case '\'':
      LBS_EmitChar(lbs, '\\');
...
static size_t _SQL_PackValue(CONVOPT *opt, unsigned char *p, ValueStruct *value,
                             Bool *fFirst) {
  int i;
  unsigned char *pp;
  LargeByteString *lbs;
...
extern size_t SQL_PackValue(CONVOPT *opt, unsigned char *p,
                            ValueStruct *value) {
  Bool fFirst;
  size_t ret;
...
static size_t _SQL_SizeValue(CONVOPT *opt, ValueStruct *value, Bool *fFirst) {
  int i;
  size_t ret;
  char *str;
...
static unsigned char *RFC822_SkipNext(CONVOPT *opt, unsigned char *p) {
  switch (opt->encode) {
...
static size_t EncodeString(CONVOPT *opt, char *out, unsigned char *in) {
  size_t result;
...
static size_t DecodeString(CONVOPT *opt, unsigned char *out, char *in) {
  size_t result;
...
static size_t _RFC822_UnPackValueNoNamed(CONVOPT *opt, unsigned char *p,
                                         ValueStruct *value,
                                         unsigned char *buff) {
  int i;
  unsigned char *q, *pp, ch;
  size_t len;
  ...
  if (value != NULL) {
    ...
    case GL_TYPE_RECORD:
      for (i = 0; i < ValueRecordSize(value); i++) {
        p +=
            _RFC822_UnPackValueNoNamed(opt, p, ValueRecordItem(value, i), buff);
      }
      break;
    case GL_TYPE_ALIAS:
    default:
...
static size_t _RFC822_UnPackValueNamed(CONVOPT *opt, unsigned char *p,
                                       ValueStruct *value,
                                       unsigned char *buff) {
...
static size_t _RFC822_PackValue(CONVOPT *opt, unsigned char *p,
                                ValueStruct *value, char *name, char *longname,
                                char *buff) {
  int i;
  unsigned char *str;
  unsigned char *pp;
...
static size_t _RFC822_SizeValue(CONVOPT *opt, ValueStruct *value, char *name,
                                char *longname) {
  int i;
  size_t ret;
...
static unsigned char *CGI_SkipNext(CONVOPT *opt, unsigned char *p) {
  while ((*p != 0) && (*p != '&') && (*p != '\n'))
    p++;
  if (*p != 0)
    p++;
...
static size_t _CGI_UnPackValue(CONVOPT *opt, unsigned char *p,
                               ValueStruct *value) {
...
static size_t _CGI_PackValue(CONVOPT *opt, unsigned char *p, ValueStruct *value,
...
extern size_t CGI_PackValue(CONVOPT *opt, unsigned char *p,
                            ValueStruct *value) {
  char longname[SIZE_LONGNAME + 1];
  size_t ret;
...
static size_t _CGI_SizeValue(CONVOPT *opt, ValueStruct *value, char *name,
                             char *longname) {
  int i;
  size_t ret;
  ...
  case GL_TYPE_ARRAY:
    ...
    for (i = 0; i < ValueArraySize(value); i++) {
      sprintf(name, "[%d]", i);
      ret += _CGI_SizeValue(opt, ValueArrayItem(value, i), name + strlen(name),
                            longname);
    }
    break;
  case GL_TYPE_RECORD:
    ...
    for (i = 0; i < ValueRecordSize(value); i++) {
      sprintf(name, ".%s", ValueRecordName(value, i));
      ret += _CGI_SizeValue(opt, ValueRecordItem(value, i), name + strlen(name),
                            longname);
    }
    break;
  case GL_TYPE_ALIAS:
