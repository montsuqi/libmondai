	...
	char		*buff)
{
	...
	if		(  value  !=  NULL  ) {
		...
		switch	(ValueType(value)) {
		  ...
		  case	GL_TYPE_DBCODE:
		  case	GL_TYPE_BYTE:
			SetValueString(value,buff,opt->coding);
			break;
		  case	GL_TYPE_ARRAY:
	...
	char		*buff)
{
	...
	if		(  value  !=  NULL  ) {
		switch	(value->type) {
		  ...
		  case	GL_TYPE_BYTE:
		  case	GL_TYPE_BOOL:
			CSV_Encode(ValueToString(value,opt->coding),fSsep,buff);
			if		(  fSsep  ) {
				p += sprintf(p,"\"%s\",",buff);
			} else {
				if		(  IsComma(buff)  ) {
		  ...
		  case	GL_TYPE_FLOAT:
			if		(  fNsep  ) {
				p += sprintf(p,"\"%s\",",ValueToString(value,opt->coding));
			} else {
				p += sprintf(p,"%s,",ValueToString(value,opt->coding));
			}
			break;
...
dbgmsg(">_CSV_SizeValue");
	...
	switch	(value->type) {
	  ...
	  case	GL_TYPE_INT:
	  case	GL_TYPE_FLOAT:
		ret = strlen(ValueToString(value,opt->coding)) + 1;
		if		(  fNsep  )	ret += 2;
		break;
	  case	GL_TYPE_ARRAY:
		ret = 0;
	...
	if		(  value  !=  NULL  ) {
		switch	(ValueType(value)) {
		  ...
		  case	GL_TYPE_FLOAT:
			...
			ch = *p;
			*p = 0;
			SetValueString(value,q,opt->coding);
			*p = ch;
			p ++;
			break;
		  case	GL_TYPE_ARRAY:
				...
				if		(  ( e = GetItemLongName(value,vname) )  !=  NULL  ) {
					...
					switch	(ValueType(e)) {
					  ...
					  case	GL_TYPE_BYTE:
						len = DecodeBase64(buff,q,p-q);
						buff[len] = 0;
						SetValueString(e,buff,opt->coding);
						break;
					  case	GL_TYPE_BOOL:
					  case	GL_TYPE_NUMBER:
					  case	GL_TYPE_INT:
	...
	if		(  value  !=  NULL  ) {
		...
		switch	(ValueType(value)) {
		  ...
		  case	GL_TYPE_FLOAT:
			...
			p += sprintf(p,"%s\n",ValueToString(value,opt->coding));
			break;
		  case	GL_TYPE_ARRAY:
			for	( i = 0 ; i < ValueArraySize(value) ; i ++ ) {
				sprintf(name,"[%d]",i);
...
dbgmsg(">_RFC822_SizeValue");
	...
	switch	(ValueType(value)) {
	  ...
	  case	GL_TYPE_BYTE:
		...
		ret += EncodeLengthBase64(ValueToString(value,opt->coding));
		break;
	  case	GL_TYPE_BOOL:
	  case	GL_TYPE_NUMBER:
	  case	GL_TYPE_INT:
	...
	char		*longname)
{
	...
	if		(  value  !=  NULL  ) {
		switch	(value->type) {
		  ...
		  case	GL_TYPE_INT:
		  case	GL_TYPE_FLOAT:
			q = ValueToString(value,opt->coding);
			p += sprintf(p,"%s=",longname);
			while	(  *q  !=  0  ) {
				if		(  *q  ==  0x20  ) {
					*p ++ = '+';
...
dbgmsg(">_CGI_SizeValue");
	...
	switch	(ValueType(value)) {
	  ...
	  case	GL_TYPE_INT:
	  case	GL_TYPE_FLOAT:
		ret = EncodeStringLengthURL(ValueToString(value,opt->coding)) + strlen(longname) + 2;
		break;
	  case	GL_TYPE_ARRAY:
		ret = 0;
		for	( i = 0 ; i < ValueArraySize(value) ; i ++ ) {
