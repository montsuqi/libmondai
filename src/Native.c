...
extern size_t NativeSaveValue(unsigned char *p, ValueStruct *value,
                              Bool fData) {
  size_t size;
  int i;
  unsigned char *pp;
  ...
  if ((type = *(PacketDataType *)p) != GL_TYPE_NULL) {
    ...
    *ret = value;
    ...
    case GL_TYPE_ARRAY:
      ...
      p += sizeof(size_t);
      ValueArrayItems(value) =
          (ValueStruct **)xmalloc(sizeof(ValueStruct *) * size);
      for (i = 0; i < ValueArraySize(value); i++) {
