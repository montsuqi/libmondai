...
extern void *_xmalloc(size_t size, char *fn, int line) {
  ...
  total += size;
  printf("xmalloc %s(%d),size = %d,", fn, line, size);
  fflush(stdout);
  ...
  *sizea = size;
  ...
  if ((ret = malloc(size)) == NULL) {
    MonErrorPrintf("no memory space!! %s(%d), request size (%zd)", fn, line,
                   size);
  }
#endif
  return (ret);
...
static void _Release(char *name, POOL *pool, void *dummy) {
  _ReleasePool(pool);
}

extern void ReleaseAllPool(void) {
  if (PoolTable != NULL) {
    LockWrite(PoolTable);
