...
extern Numeric NumericRescale(Numeric num, int precision, int scale) {
  ...
  if (num->n_weight < maxweight && scale >= num->n_rscale) {
    ...
    new->n_rscale = scale;
    new->n_sign_dscale = NUMERIC_SIGN(new) | ((int)scale & NUMERIC_DSCALE_MASK);
    return new;
  }
...
extern Numeric NumericRound(Numeric num, int scale) {
   ...
   * ----------
   */
  scale = MIN(NUMERIC_MAX_RESULT_SCALE, MAX(-NUMERIC_MAX_RESULT_SCALE, scale));
...
static void dump_var(char *str, NumericVar *var) {
  ...
  printf("%s: VAR w=%d r=%d d=%d ", str, var->weight, var->rscale, var->dscale);
  switch (var->sign) {
  case NUMERIC_POS:
    printf("POS");
    break;
...
static void SetVarFromNum(Numeric num, NumericVar *dest) {
  NumericDigit *digit;
  int i;
  int n;
...
static Numeric MakeResult(NumericVar *var) {
  ...
  result->n_sign_dscale = sign | ((int)var->dscale & NUMERIC_DSCALE_MASK);
...
static void SqrtVar(NumericVar *arg, NumericVar *result) {
  ...
  if (stat < 0)
    fprintf(stderr,
            "math error on numeric - cannot compute SQRT of negative value");
...
extern void NumericFormat(char *buf, char *format, Numeric num) {
  char *f, *p, sup;
  size_t len;
  int fPSign, fNSign, fMinus, fMark, fSup, fSmall;
  char *str, *q, *qq;
  Numeric nnum;
