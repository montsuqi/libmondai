...
static guint NameHash(gconstpointer key) {
  char *p;
  guint ret;
...
static gint NameCompare(gconstpointer s1, gconstpointer s2) {
  if (s1 == NULL) {
    return -1;
  }
  if (s2 == NULL) {
...
static guint NameiHash(gconstpointer key) {
  char *p;
  guint ret;
...
static gint NameiCompare(gconstpointer s1, gconstpointer s2) {
...
static void _ClearNames(gpointer key, gpointer value, gpointer user_data) {
  xfree(key);
}
...
static guint IntHash(gconstpointer key) { return ((guint)(long)key); }

static gint IntCompare(gconstpointer s1, gconstpointer s2) {
...
extern void DestroyHashTable(GHashTable *hash) {
  if (hash == NULL) {
    return;
  }
