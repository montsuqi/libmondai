...
static TokenTable tokentable[] = {{"bool", T_BOOL},
                                  {"byte", T_BYTE},
                                  {"binary", T_BINARY},
                                  {"char", T_CHAR},
                                  {"varchar", T_VARCHAR},
...
extern ValueStruct *ParValueDefine(CURFILE *in) {
  ...
  case T_TEXT:
    ...
    if (GetSymbol == '(') {
      ...
      if ((token == T_DBCODE) || (token == T_TEXT)) {
        fExpandable = TRUE;
      } else {
        size = 1;
      }
...
static void ParValueDefines(CURFILE *in, ValueStruct *upper) {
  ...
  if ((!in->fError) && ((ComToken == '}') || (ComToken == ')'))) {
    GetSymbol;
    /*	OK	*/
  } else {
    printf("token = %d [%c]\n", ComToken, ComToken);
...
static ValueStruct *_RecParseValue(CURFILE *in, char **topname) {
